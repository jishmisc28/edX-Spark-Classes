...
# TODO: Replace <FILL IN> with appropriate code
wordCountsCollected = (wordsRDD.map(lambda k:(k,1)).reduceByKey(lambda a,b: a+b).collect())
...
# TODO: Replace <FILL IN> with appropriate code
uniqueWords = wordsRDD.map(lambda k:(k,1)).reduceByKey(lambda a,b: a+b).count()
print uniqueWords
...
totalCount = (wordCounts.map(lambda (x,v):v).reduce(lambda a,b:a+b))
average = totalCount / float(wordCounts.reduceByKey(lambda a,b:a+b).count())
print totalCount
...
def wordCount(wordListRDD):
    ...
    """
    wordsRDD2 = wordListRDD.map(lambda x: (x,1)).reduceByKey(lambda a,b:a+b)
    return wordsRDD2
