...
print 'hosts: %s' % uniqueHosts.count()
...
dayHostCount = dayGroupedHosts.map(lambda ((k1,k2),v):(k1,v))
...
avgDailyReqPerHost = (sortedByDay.map(lambda (x, y):(x,(len(y)/len(set(y))))).cache())
...
badEndpointsTop20 = badEndpointsSum.takeOrdered(20, key = lambda s: -1 * s[1])
...
errHostsTop25 = errHostsSum.takeOrdered(25, key = lambda s: -1 * s[1])
