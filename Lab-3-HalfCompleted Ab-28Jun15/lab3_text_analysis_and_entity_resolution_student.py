...
def simpleTokenize(string):
    ...
    return filter(None,re.split(split_regex,string1))
...
def tokenize(string):
    ...
    """
    list0 = ''.join([w.lower() for w in string])
    #list0 = [w.lower() for w in string]
    list1 = filter(None, re.split(split_regex,list0))
    return filter(lambda x:x not in stopwords,list1)
...
def countTokens(vendorRDD):
    ...
    """
    vendorRDD1 = vendorRDD.map(lambda (k,v):len(v)).reduce(lambda a,b:a+b)
    return vendorRDD1
